...
interface OverviewProps<TData, TValue> {
  ...
  selectable?: boolean;
  actions?: (utils: {
    services: BaseApiCallProps<TValue>;
    onSubmit: () => void;
    selectedRows: TData[];
  }) => React.ReactNode[];
  rowActions?: (cell: {
...
export default function OverviewPage<TData, TValue>({
      ...
      const sort = sorting?.[0]
        ? (() => {
            ...
            const accessorKey = String(
              column?.columnDef && "accessorKey" in column.columnDef
                ? column.columnDef.accessorKey
                : undefined,
            );
            return accessorKey
              ? `${accessorKey},${sorting[0].desc ? "desc" : "asc"}`
              : "";
          })()
        : "";
  ...
  return (
    <div
      className={`flex flex-col gap-4 py-4 md:gap-6 md:py-6 h-full select-none ${loading ? "cursor-progress" : undefined}`}
    >
      <div className="px-4 lg:px-6">
        <DataTable
          ...
          actions={
            actions
              ? actions({ services: source, onSubmit: refresh, selectedRows })
              : []
          }
          rowActions={
            rowActions ? ({ row }) => rowActions({ row, id, setId }) : undefined
          }
          sorting={sorting}
