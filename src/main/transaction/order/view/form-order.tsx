...
export default function FormOrder(props: BaseForm) {
    ...
    React.useEffect(() => {
        form.setValue('address_id', String(shippingParameter?.find(item => item.address_flag.includes('default_address'))?.address_id))
    }, [shippingParameter]);

    React.useEffect(() => {
        form.setValue('pickup_time_id', String(pickupTime?.find(item => item.flags.includes('recommended'))?.pickup_time_id))
    }, [pickupTime]);
    ...
    return <Form {...form}>
        <form onSubmit={form.handleSubmit(submitForm, console.log)}
            ...
            <div className="columns-1 lg:columns-2 gap-4">
                <div className="px-8 mt-4 mb-2 text-sm break-inside-avoid">
                                            ...
                                            <SelectTrigger aria-invalid={fieldState.invalid} className="w-full">
                                                <div className="flex w-full text-left">
                                                    {(() => {
                                                        ...
                                                        </div> : <p className="text-muted-foreground">Pilih alamat</p>
                                                    })()}
                                                </div>
                                            </SelectTrigger>
                                            <SelectContent>
                                                {shippingParameter && shippingParameter.map((item, index) => {
                                                    return <SelectItem key={index} value={String(item.address_id)} className="[&>span]:flex-1">
                                                        <div className="flex-1 flex">
                                                            <div className="flex flex-col flex-1">
                                                                <div>Alamat: {item.address != '' ? item.address : '-'}</div>
                                                                <div>Provinsi: {item.state != '' ? item.state : '-'}</div>
                                                                <div>Kota: {item.city != '' ? item.city : '-'}</div>
                                                                <div>Kecamatan: {item.district != '' ? item.district : '-'}</div>
                                                                <div>Kelurahan: {item.town != '' ? item.town : '-'}</div>
                                                                <div>Kode Pos: {item.zipcode != '' ? item.zipcode : '-'}</div>
                                                            </div>
                                                            <div className="flex flex-0 flex-col gap-2">
                                                                {item.address_flag.map((item, index) => (
                                                                    <Badge variant="success" key={index}>{item.replaceAll('_', ' ')}</Badge>
                                                                ))}
                                                            </div>
                                                        </div>
                                                    </SelectItem>
                                            ...
                                            <SelectTrigger aria-invalid={fieldState.invalid} className="w-full">
                                                <div className="flex w-full text-left">
                                                    {(() => {
                                                        ...
                                                        return value ? <div className="flex gap-2">
                                                            <p>{value.time_text}</p>
                                                        </div> : <p className="text-muted-foreground">Waktu pengambilan</p>
                                                    })()}
                                                </div>
                                            </SelectTrigger>
                                            <SelectContent>
                                                {pickupTime && pickupTime.map((item, index) => {
                                                    return <SelectItem key={index} value={item.pickup_time_id} className="[&>span]:flex-1">
                                                        <div className="flex-1 flex">
                                                            <div className="flex flex-col flex-1">
                                                                <div>{item.time_text != '' ? item.time_text : '-'}</div>
                                                            </div>
                                                            <div className="flex flex-0 flex-col gap-2">
