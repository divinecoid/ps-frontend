...
export default function TrackCMT() {
    ...
    const handleSearch = async () => {
        ...
        setIsError(false);
        try {
            const response = await Services.TransactionRequest.searchCmt(modelId, colorId, sizeId);
            if (!response.ok) {
    ...
    return (
        <div className="flex flex-col flex-1 p-4 md:p-8 gap-4 overflow-y-auto">
            <h1 className="text-2xl font-bold tracking-tight">Lacak CMT</h1>
            
            <Card>
                <CardHeader>
                    <CardTitle>Filter Pencarian</CardTitle>
                    <CardDescription>Pilih Model, Warna, dan Ukuran untuk mengetahui di CMT mana barang tersebut sedang diproses.</CardDescription>
                </CardHeader>
                                ...
                                <TableBody>
                                    {results.map((row, index) => (
                                        <TableRow key={row.request_id}>
                                            ...
                                            <TableCell>{row.cmt_name}</TableCell>
                                            <TableCell>{row.req_qty}</TableCell>
                                            <TableCell>{row.sisa_qty}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
